...
const REMINDER = `[Cypher Tempre] ACTIVE — run the loop and seal before ending: python3 ${SKILL_DIR}/recall.py turn "<finding>" --input "<request>". Route task-shaped requests first: python3 ${SKILL_DIR}/router.py route "<task>". Score your own seals (--coherence/--relevance/--novelty/--consistency/--depth/--covenant, 0-255) and declare --used-rings on grounded claims — the lexical proxies are fallbacks, not judges. Fork per the doctrine if a genuine either/or decision exists. Context truncates to the first user message + recent turns; the chain is memory — recall, don't assume. (Skip only if dormancy.py status says dormant.)`
